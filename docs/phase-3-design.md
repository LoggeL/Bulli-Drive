...
**Im Rennbetrieb (Schritt „tracks-and-arena“, A55):** Alle sechs Strecken stehen in `TRACK_IDS`, auch die Bonus-Strecken Dune Rally und Grand Tour, weil sie fahrbar sind: Rotation und Lobby in der Reihenfolge der Tabelle. Fünf Server-Bots (Stufe mittel) fahren jede Strecke im Room komplett durch (`tests/integration/trackRaces.test.ts`); gemessen über 20 Seeds je Strecke (100 Bot-Fahrten je Strecke) im Mittel 2:02 (Downtown Loop, drei Runden), 0:57, 1:10, 2:27 (Harbor Circuit, drei Runden), 1:16 und 2:38, kein DNF, längster Stillstand 9,6 s.
...
- 180 × 140 m bei (−170 | +590), Beton/rissiger Asphalt, Maschendrahtzaun ringsum (`segment`, top ∞), ein Tor zum Harbor Blvd ~~, das im PartyRoom geschlossen ist (Collider nur in der Party-Welt)~~ (steht in der Party offen, A54).
...
- **Party-Zone (A54):** Die Party reicht über die Arena hinaus in die Hafenhöfe rundherum: Rechteck (−331 | 500) bis (−9 | 728) mit Dock Street, Wharf Road, Foundry Road, den Gassen zwischen den Lagerhallen und dem Stück Cannery Lane vor dem Tor. Dort liegen 21 weitere Coins, 8 Power-ups und 8 Spawn-Plätze (Gruppe `harbor`) auf den Straßen. Die Party-Welt hat einen Zaun um die Zone (wo eine Halle auf der Linie steht, schließt ihre Wand die Zone), die Weltgrenze ist die Zone, das Tor der Arena ist offen.
...
- **Ghosts:** Ghosts des Zeitfahrens sind an `trackVersion` gebunden und werden mit dem Port ungültig. Persistente Bestenlisten gibt es erst in Phase 4, also geht nichts Dauerhaftes verloren. Der Server verwirft beim Start jeden Lauf, dessen Schlüssel (Strecke, `trackVersion`, `mapVersion`, Sim-Hash) keine heutige Strecke trifft (`GhostStore.retain`, Teil des Store-Vertrags, damit der SQLite-Store aus Phase 4 dasselbe tut; A55).
...
**Stand (Schritt „rebase-and-sim“, M3):** `tests/shared/sim/mapColliders.test.ts` (Hüllboxen von Kapsel und gedrehter Box, Gitter über 2 km, Kapsel steht auf dem höheren Ende, obox bei Gier 0 gleich der achsparallelen Box, Herausdrücken entlang der Flächennormale einer um 30° gedrehten Box, Leitplanke: Gleiten bei 10° und 50 m/s, Überspringen, kein Stehen darauf, Tunneling aller Klassen bis 90 m/s) und `surfaces.test.ts` (Rollwiderstand von Hand: erster Tick auf Sand 8,5 − 1,6 · 0,8 m/s², Oberfläche je Achse, Seitenkraft proportional zum Griff, Endgeschwindigkeit auf Sand, Waten, Wasser: kein Antrieb, Dämpfung, Reset nach genau 60 Ticks, nichts beim Überfliegen, kein Boost und kein Sprung; Fallgrenze). `tests/shared/map/mapData.test.ts` prüft Lose, Pflanzen, Zaun, Kartenrand, Reset auf die Straße und den Heightfield-Hash an kleinen Handkarten und die Invarianten der echten Karte (Gebäude außerhalb aller Korridore und Areas, in ihrer Zone, auf ebenem trockenem Boden, überlappungsfrei; Pflanzen nicht auf der Fahrbahn; Party-Ghost bleibt in der Arena; Hash gleich bei gleichem Bau, anders bei verschobener Münze und Höhe), `structures.test.ts` die Footprints gegen das Kit-Manifest und die Box-Tests, `tests/shared/race/tracks.test.ts` die portierten Strecken in ihren Rennwelten (Ideallinie ≥ 2,5 m von jedem Collider, Startaufstellung auf der Fahrbahn und frei, Steigung ≤ 15 %, Rampen), `tests/server/spawn.test.ts` die Spawn-Slots, `tests/tools/map/driveTrack.test.ts` fährt jede Strecke in der echten Rennwelt. `npm run test:engines` vergleicht zusätzlich den `worldHash` in WebKit und Chromium mit Node (bitgleich).
...
**Stand (Schritt „tracks-and-arena“):** `tests/integration/trackRaces.test.ts` fährt jede der sechs Strecken mit drei Seeds im Room: fünf Server-Bots mit Kontakt, der Spieler parkt abseits; jeder Bot kommt ins Ziel oder ist beim DNF noch unterwegs, keiner steht länger als 10 s ohne Fortschritt (18 Rennen in 4 s, ohne Sockets, Teil von `npm run test:bots`). Unit: `routeToTrack.test.ts` (Rundkurs beginnt am Start, Handrechnung am Quadrat), `gates.test.ts` (Slalom: jedes Gate auf dem nächsten Ast voraus), `geometry.test.ts` (Projektion „voraus“ über die Naht), `progress.test.ts` (Reset weit hinter dem eigenen Platz), `pursuit.test.ts` und `lineDriver.test.ts` (Stillstand zählt für Renn-Bots unabhängig vom Pedal, kurzes Abprallen ist kein Losfahren, Traktionskontrolle), `tracks.test.ts` (Gates in Reihenfolge entlang der Ideallinie, Steigung je Belag), `mapData.test.ts` (`zoneFence` von Hand, Party-Zone mit offenem Tor und Zaun über Cannery Lane, Ghost an der Grenze), `validateMap.test.ts` und `mapFiles.test.ts` (Zone, Hof-Spawns und -Items), `timeTrial.test.ts` und der Store-Vertrag (`retain`), `driver.test.ts` (Last-Bots aus den Höfen), `raceUi.test.ts` (sechs Strecken in der Lobby).

**Stand (Schritt „client-world“, M4):** Unit, ohne Renderer: `terrainGrid.test.ts` (Ringe gegen `heightAt` auf Bulli Bay: 5 cm bis 25 m, 3 cm auf Straßen bis 60 m; rissfreie Ränder, Hysterese, grobe Basis), `terrainSplat.test.ts`, `roadGeometry.test.ts` (Bänder, Kreuzungen, Ecken, Areas, Markierungscodes und die Aufteilung in Blöcke an Hand-Netzen), `kitCells.test.ts` (jedes Teil genau einmal, LOD je Stufe, Sichtweite), `chunkedInstances.test.ts` (Packen je Chunk, Sichtweite, Nahgrenze), `mapScene.test.ts` (jeder Collider der Karte hat sein gezeichnetes Gegenstück und umgekehrt, Kit-Platzierungen, Pier), `railings.test.ts`, `scatter.test.ts` (Abstände, Zonen, Briefkästen, Ranch-Zäune an Handkarten), `viewpoint.test.ts`, `radar.test.ts` (Achsen), `look.test.ts`, `worldQuality.test.ts`, `renderQuality.test.ts` (Erkennung des CPU-Rasterizers, Multisampling), `loadingScreen.test.ts`; Shared: `furniture.test.ts` (Laternen, Hydrant, Bänke, Ampeln einer Kreuzung, Einbahn, Freiraum, spitze Kreuzung, Bankring an Handnetzen; Invarianten der echten Karte), `buildingKit.test.ts` für den Kran. Render-Job: Budget der Handy-Stufe an Main Street, Hafen und Ridge-Kehre sowie mit sieben fremden Autos. E2E: die Welt der Karte mit Kit, Palmen-Impostors und Gruppen `map`/`sky-dome`.
...
**Stand (Schritt „tests-budget-docs“):** Die E2E-Pfade fahren auf Bulli Bay von der längsten freien Nord-Süd-Geraden (Free Roam) bzw. quer durch die Arena (Party, `tools/bots/runway.ts`); `arenaRunway` hat jetzt eigene Unit-Tests in `tests/tools/driver.test.ts` (auf der echten Karte: in der Arena, jede Münze und jedes Power-up außerhalb des Aufnahmeradius, ein Auto fährt sie ohne Wandkontakt; an einer Handarena von 100 × 12,5 m: volle Länge 88,5 m, Ende 3,97 m vor dem Kreis einer Münze, Ende am um 4,75 m gewachsenen Collider). Budget je Tier (A72): software im E2E-Desktop-Pfad (≤ 110 Calls, ≤ 300 k Dreiecke, kein Schatten-Pass), low im Render-Job an Main Street, Hafen und Ridge-Kehre sowie mit sieben fremden Autos, high als Warnung in `npm run screenshots`. Die Minimap-Achsen prüft `radar.test.ts` auf Unit-Ebene statt E2E. Laufzeit des E2E-Jobs: A73. **Stryker:** Gruppe `race` inkrementell 86,4 % (Phase 2: 84 %); jeder Überlebende auf einer seit `main` geänderten Zeile ist von Hand nachgeprüft (neue Tests für kaputte Routen in `tracks.json`, die Projektion „voraus“ auf offener Linie und die Gasschwelle 100 des Stuck-Watch, der Rest äquivalent). Gruppe `map`: Die Karten-Quellen sind seit dem letzten Lauf (82,7 %) unverändert; ein inkrementeller Lauf hätte wegen der Test-Zuordnung von Vitest 5 (52 Testdateien „geändert“) über 6 000 Mutanten neu getestet, geschätzt 5 h, und ist abgebrochen. Stattdessen `mapData.ts` von Hand gegen alle Tests, die die Karte bauen: von 182 gemeldeten Überlebenden waren 99 echt, neue Tests (Collider je Landmarke, Container, Brunnen und Pflanze, Party ohne Zone, Zaun an Container und Landmarke, Reset an Dreiecksfläche, weit weg und auf diagonaler Straße, Items, Rand, Fehlerfälle) lassen 43 äquivalente übrig. Die Gruppe lief vorher nicht mehr an: Die Sandbox ließ `public/models/kit/manifest.json` weg (jetzt in `stryker.config.mjs` behalten).
...
Stand nach den Schritten „Straßennetz und Heightfield“ (A1–A12), „curated-map-data“ (A13–A20), „worldviewer“ (A21–A22) und „building-kit“ (A23–A27), alle M1, Module ohne Integration, dem Schritt „rebase-and-sim“ (A42–A53: M3, Sim und Server auf Bulli Bay), dem Schritt „tracks-and-arena“ (A54–A58: Strecken, Bots, Party-Zone), dem Schritt „client-world“ (A59–A70: M4, der Client zeichnet Bulli Bay) und dem Schritt „tests-budget-docs“ (A71–A73: Tests, Budgets, Laufzeit der E2E-Suite):
...
| A28 | 5.3, E4 | Auch die Kurven, Leitplanken und Routen rechnen nur mit exakt gerundeten Operationen (`Math.sqrt` statt `Math.hypot`), Collider-Enden werden auf 1 mm, Gate- und Grid-Winkel aus `atan2` auf 1 µrad gerundet. Ein Regeltest verbietet genäherte `Math`-Funktionen in allen Modulen auf dem Weg zu Sim, Hash und Bake. Dazu vergleicht `npm run test:engines` (Playwright, eigene Konfiguration und CI-Job, ca. 10 s) in WebKit und Chromium einen Fingerabdruck aus `heightAt` an 20 000 Punkten, den rohen Straßen-Samples, den Leitplanken-Collidern, den `TrackDef`s aller Strecken und dem ganzen Bake mit Node: bitgleich. | Befund aus dem Review: `hypot` und `atan2` können in WebKit im letzten Bit abweichen; über die Collider hätte das den strikt verglichenen `worldHash` verändert und iOS-Clients in eine Reload-Schleife geschickt. |
...
| A40 | 6.4, 15 | **Suchradius des Shapers und Mutationstests der Bake-Module.** `TerrainShaper.reach` prüfte nur die 32-m-Blöcke innerhalb des Anfangsradius und übersah höheres Gelände dahinter; jetzt zählt jeder Block bis 240 m mit seinem Minimum und Maximum (Bedarf = ebene Zone + 1,5 × größter Höhenunterschied zur Straße, gilt, sobald er den Abstand des Blocks erreicht). Das Bake ändert sich an 270 von 1 002 001 Rasterpunkten um bis zu 8,8 m (Böschungen hinter hohen Blöcken), `BAKE_VERSION` 2. Neue Tests auf der untersten Ebene: `pinEnvelope` (Pins auf einer Station in beiden Reihenfolgen, schwächere Pins zwischen Stationen, Pins hinter dem Ende, Steigung je Schritt vorwärts), Längsprofil mit fallender und steigender Steigungsgrenze, Shaper bei 2 m Abstand, Seiten und Breiten abseits des Ursprungs, wachsende Breite, Wand- und Oberflächen-Flag des näheren Stützpunkts, Vorrang und Breite der Oberflächen, Areas auf allen Seiten, Ort des schlimmsten Konflikts; Bake-Teile (`bakeParts.test.ts`: Rasterabfrage, Area-Mittel, Korridorlinie mit Wänden und rückwärts befahrenen Kanten, Plateaus an Kreuzungen und Areas in beiden Richtungen, Ringe, Bericht, Bodenklassen). Stryker: `corridor.ts` 74 → 83 %, `bakeTerrain.ts` 53 → 81 %. Die übrigen Mutanten sind einzeln geprüft und fallen in diese Klassen: (1) `<` gegen `<=` an stetigen Werten (Höhen, Abstände, Schwellen der Bodenklassen), Gleichheit kommt im Bake nicht vor und beide Seiten wären richtig; (2) Schleifengrenzen und -starts außerhalb des Arrays (`i <= n`, `n + 2`): typisierte Arrays ignorieren Schreiben dahinter, Lesen gibt `undefined`, jeder Vergleich mit NaN ist falsch; (3) NaN-Leerläufe an Kettenenden (Kegel bei `a < 2`, Segment hinter dem letzten Stützpunkt, Segment der Länge 0); (4) größere Suchradien (Blockabstand, Bounding-Box ±∞, `dist > radius`): kosten nur Zeit, außerhalb des Kegels bindet keine Schranke; (5) redundante Schutzschritte: Klemmen der Basis in [lo, hi] (die Rundung bleibt im Steigungskegel der Pins), Innereien der Bisektion (Startintervall, Toleranz, Abbruch, Verteilung des Rests unter 1e-12); (6) Pfade, die der bytegleiche Bake-Test abdeckt, den Stryker aus Zeitgründen auslässt (instrumentiert ≈ 45 s je Bake): einzeln angewandt tötet er 33 der damals 96 überlebenden `bakeTerrain`-Mutanten; (7) nur Bericht: Zeitmessung, `maxBakedGrade` bei 1-m-Samples (`*` und `/` gleich), Schwelle 1 cm der Liste; (8) Topologien, die Bulli Bay nicht hat: Ringe in den Knotenmitteln, Knoten zweier Areas, Ketten unter 30 m für das Glättungsfenster (alle Ketten haben Stationsabstand genau 1 m). Zu (8): Ein Ring durch eine Kreuzung nähme heute sein eigenes Mittel statt der Knotenhöhe; das wird beim ersten Ring der Karte korrigiert und getestet. | Befund 15 aus dem Review („Known gaps“ ohne Test und ohne Begründung). |
| A41 | 10, 11.1, 14.1 | **Gebäude-Kit, zweite Runde.** Main Street in vier Stilen (`style`): Backstein nur noch 2 von 14 Häusern, Putz 5, Mission Revival 3 (Kalkputz, Bogenfenster und -tür, Ziegel-Vordach an der Brüstung, geschwungener Mittelgiebel), Art déco 4 (Pastellputz, Lisenen an den Achsgrenzen über einem gestuften Mittelaufsatz, drei Speed Lines), 1–3 Geschosse; Mission und Deco als eigene Gruppe `downtown_revival` (Bytegrenze je GLB). 14 statt 6 Ladenschilder (zweiter KI-Bogen, Plätze 256 × 63 px, ca. 66 px/m), 11 Gewerbe auf den 4 Ladenfronten, Nachbarn verschieden. Neue Gruppe `landmarks`: Leuchtturm, Wasserturm, Diner, Tankstelle. Alle LODs haben dieselbe Texeldichte (Test je Kachel-Region); dafür steigen LOD2 für Gebäude auf 1 000 Dreiecke, der Anteil des gröbsten LODs auf 60 % (ausgenommen Teile, deren LOD0 schon ins LOD2-Budget passt), die Bytegrenzen auf 440 KB je Gruppe und 2,4 MB gesamt (heute 2,06 MB). **LOD und Merging:** Nach dem Zusammenführen eines 250-m-Chunks schaltet kein Teil mehr sein LOD; bis `BatchedMesh` mit dem Three-Upgrade (M2) kommt, führt der Chunk-Builder je LOD-Stufe Unterzellen von 62,5 m zusammen und schaltet sie als Ganzes nach den Kategorie-Distanzen. **Vegetation** (Palmen, Monterey-Zypresse, Eiche, Chaparral) folgt als eigene Gruppe mit einem zweiten Material `kit_foliage` (Alpha-Test, Blattkarten-Atlas), zusammen mit der Bestückung aus 11.2; ein Schild je Instanz (UV-Tausch im Chunk-Builder) mit M4. Review-Renderings unter einem physikalischen Himmel ohne Landschaft statt des Kapstadt-HDRIs. | Befund 11 aus dem Review. |
| A42 | 14.2 | **M3 zusammen mit dem Server-Teil von M5.** Sim und Server laufen auf Bulli Bay, der Client sagt die Karte voraus, zeichnet aber bis M4 noch die alte Stadt (nur Optik; Pickups, Effekte und wartende Autos stehen schon auf dem Boden der Karte). Die alte Stadt verschwindet aus Server und Sim (`createMapData` der Stadt, `spawn.ts` mit Zufallspunkten, `mapFeatures.ts`, die Phase-2-Strecken und ihre Hints); `cityGen.ts`, `terrain.ts`, `worldGen.ts`, `colliderGen.ts` bleiben für die Optik des Clients bis M4. `mapVersion` bleibt 4 (A38) und ist damit die nächste Version nach der Stadt (2) und ihren Rennen (3). Protokoll v4: `roomState.world` nennt `mapId` statt eines Seeds, der Self-Block trägt `waterTicks`. | Der Schritt verlangt die Umstellung von Sim und Server; der Branch geht erst mit M5 live, dazwischen darf die Optik hinterherhinken. |
| A43 | 5.1, 6.2, E13, A37 | **Laden der Karte.** `createMapData(sources, hf)` (`src/shared/map/mapData.ts`) baut auf beiden Seiten dieselben Daten; die Quellen prüft `mapSources.ts`. Der Server liest die JSON-Dateien und `terrain.bhf` von der Platte (`src/server/maps.ts`; im Build kopiert `scripts/copy-map-sources.mjs` die Quellen nach `dist/shared/maps`, weil `tsc` nur importierte Module ausgibt und das Docker-Image nur `dist/` enthält), der Client bündelt die JSON-Dateien und lädt `terrain.bhf` über den Hash aus `manifest.json`, bevor er die Verbindung öffnet. Die Vorkomprimierung für `*.bhf` kommt schon jetzt (nicht erst mit M4): Brotli 361 KB statt 3 MB. `src/shared` bleibt ohne JSON-Imports und damit ohne Dateizugriff; der Server braucht keine JSON-Module (Import-Attribute) in Node. | Der Client lädt die Datei ab jetzt; ohne Kompression wären es 3 MB. |
| A44 | 8.2, E8 | **Collider.** `obox` speichert statt eines Winkels die Einheitsachse seiner lokalen z-Achse (`ux`, `uz`): Lose und Pflanzen rechnen ganz ohne Trigonometrie, nur die Posen aus `pois.json` gehen einmal über `sin`/`cos`, auf 10⁻⁶ gerundet (`yawAxis`). Eine Kapsel steht auf dem höheren ihrer Enden (am Hang ist die Leitplanke nie niedriger als `top`) und trägt kein Auto (zu schmal zum Draufstehen). Das Gitter ist je Welt parametrisiert (Karte: −1000 m, 16 m, 125 × 125). Die Weltgrenze ist ein Rechteck (`border`); in der Party ist es der Zaun der Arena, damit auch ein Party-Ghost (ohne Welt-Collider) sie nicht verlässt. | Exakt gleiche Collider in jeder Engine; die Arena war für Ghosts offen (im Lasttest gefunden). |
| A45 | 7, 8.1 | **Oberflächen und Wasser.** Befestigte Flächen haben keinen Rollwiderstand (auch Holz nicht; wie `drivability.ts` rechnet), unbefestigte `d · offroadDrag`; der Griff gilt je Achse an ihren Kreismittelpunkten. Wasser (Meeresboden unter flachem Wasser) fährt wie weicher Sand: Griff 0,55, Wat-Widerstand 3 m/s²; `drivability.ts` behandelt Wasser weiter als unbefahrbar. „Im Wasser“ ist ein Auto, dessen Unterseite 0,6 m unter dem Wasserspiegel liegt (nicht die Wassertiefe am Mittelpunkt): wer über das Wasser springt, zählt erst, wenn er eintaucht. Dann kein Antrieb, kein Boost, kein Sprung, Dämpfung 2,5/s, nach 60 Ticks der Reset; der Zähler `waterTicks` gehört zum Zustand und zum Self-Block. Unter y = −20 folgt der Reset sofort. Goldens neu erzeugt: einziger Unterschied das Feld `waterTicks: 0`. | Ohne Griff im Flachwasser wäre der Strand nicht befahrbar (Division durch 0 im Reifenkreis); ohne Zähler im Snapshot sagte der Client den Reset falsch voraus. |
| A46 | 8.3 | **Reset auf die Straße.** Nächster Punkt der Mittellinie (Suche 60, 400, dann 3000 m), die Richtung, die dem Gier am nächsten liegt (Einbahn nur ihre), die Mitte der rechten Hälfte einer Zweirichtungsstraße. Auf einer Area bleibt das Auto, wo es ist (statt der Mitte der Area). Kosten 14 µs, nur beim Reset. | Areas bis 180 × 140 m: die Mitte liegt oft weit weg oder in einer Landmarke; auf einer Area steht das Auto schon auf befahrbarem Grund. |
| A47 | 11.2, 11.1, A23 | **Bestückung.** Lose nur an befestigten Straßen, je Zone mit den Kit-Teilen: Downtown geschlossene Zeilen (Front 0,3 m hinter dem Gehweg), Wohnen 6 m Rücksprung und 4–12 m Abstand, Industrie 10 m und 6–16 m, Strand 3 m und 12–28 m; ein verworfenes Los rückt 2 m weiter. Ein Los bleibt 0,25 m außerhalb jedes Korridors (Fahrbahn + Gehweg der Seite + Bankett). Pflanzen auf einem gestreuten Raster je Zone (Wildnis, Hügel, Ranch, Klippen, Wohnen, Park, Strand) mit Kollidern für Eiche, Zypresse, Palme und zwei Felsgrößen, dazu Palmen alle 18 m an Main Street und auf der Promenade des Ocean Boulevard; 2 m + Stamm vom Korridor, 1,5 m + Stamm von Gebäuden. Landmarken mit Kit-Teil: Diner, Tankstelle, Leuchtturm, Wasserturm, Rettungstürme, Lichtmasten; Cannery, Scheune, Pier-Restaurant und Surf-Shop mit Halle bzw. Strandhaus als Platzhalter; Kräne ohne Collider bis zu ihrem Modell (M4; *erledigt mit A68*). Freigehalten werden Landmarken, Rampen mit 45 m Landung und Spawns. Ergebnis: 549 Gebäude, 1 898 Pflanzen, 25 Landmarken und Container, 3 374 Collider; Bau 170 ms. | Die Collider gehören in `src/shared` (11.2); das Kit liefert feste Varianten statt freier Parameter (A23). |
| A48 | 5.5 | **Leitplanken enden vor Kreuzungen:** um Trimm-Radius + halbe Straßenbreite + 2 m (die Optik im worldviewer und in der Vorschau ebenso). Die Leitplanken-Pflicht der Validierung prüft weiter die Angaben in `roads.json`. | Mit den Collidern lag die Ideallinie des Ridge Climb, des Coast Sprint, der Dune Rally und der Grand Tour in Kreuzungen auf Leitplanken (der Übergang schneidet die Ecke, A30); jetzt hält jede Linie ≥ 2,7 m Abstand. |
| A49 | 12 | **Party-Arena.** Zaun 0,5 m innerhalb des Parkplatzes mit einer Lücke in Torbreite, das Tor als Kapsel nur in der Party-Welt; Container `obox` (Länge in Blickrichtung der Pose), top 2,6 m (man kann auf ihnen landen); die zwei Rampen der Arena gehören zur Karte. Münzen und Power-ups aus `pois.json`, Typen reihum. | Konkretisiert Abschnitt 12. |
| A50 | 3.5 | **Spawns** aus den Slots von `pois.json`: die Gruppen reihum, jede Gruppe ab ihrem nächsten Slot, der erste Slot mit 12 m Abstand zu Autos und außerhalb der Pickups, sonst der mit dem meisten Platz. Kein Zufall mehr. | Deterministisch und testbar. |
| A51 | 13.2, 13.3 | **Strecken.** `TrackDef` bekommt `ramps` (mit `look`); `mapTracks(map)` baut die Phase-2-IDs aus `tracks.json`. Die Rennwelt legt die Rampen der Strecke hinter die der Karte, schräge Barrieren sind `obox`. Die Renn-Bots planen ihr Tempo mit dem Griff der Oberfläche unter der Ideallinie (`Course.surfaces`); sonst fliegen sie auf der Dune Rally aus der Kurve. | Die Sim kennt jetzt Oberflächen. |
| A52 | 15 | **Tests und Bots.** Die Last-Bots fahren über das Straßennetz (befestigte Durchgangsstraßen zuerst) bzw. in der Arena in Vorwärtsbögen mit freiem Weg und jagen kein Auto hinter einem Container. In der Arena setzt der Reset das Auto nicht um; deshalb wendet ein Bot dort in drei Zügen (rückwärts mit Gegenlenkung), wenn sein Ziel hinter ihm liegt, nimmt den freiesten Weg, wenn jedes Ziel zugestellt ist, und bricht eine Jagd nach dem ersten Zurücksetzen ab. Der Bump-Runway liegt in der Party quer durch die Arena. Rennen und Zeitfahren im Integrationstest laufen gleichzeitig (der Ridge Climb dauert eine Minute). Der Render-Test des Phone-Tiers misst bis M4 in Free Roam, wo die alte Stadt sichtbar ist. | Die Laufzeitziele der Ebenen halten (CLAUDE.md). |
| A53 | 10, 15 | **Kosten im Tick** (`npm run perf:sim`, M-Mac): 32 Bots auf Bulli Bay 0,05 ms im Mittel, p99 0,1 ms (Budget p99 < 4 ms), die flache leere Welt 0,04–0,09 ms. Die Abfragen im Tick sind `heightAt`, `surfaceAt` (zwei je Auto) und das Collider-Gitter; `nearestRoad` läuft nur beim Reset. | Messung statt Schätzung (Abschnitt 15, Integration). |
| A54 | 12, 3.5 | **Party-Zone statt Arena allein.** Die Party reicht von Cannery Lot in die Hafenhöfe rundherum (`pois.json` `party`: Zone (−331 \| 500)–(−9 \| 728), 21 Coins, 8 Power-ups; 8 Spawns mit `group: "harbor"`): Dock Street, Wharf Road, Foundry Road, die Gassen zwischen den Lagerhallen, das Stück Cannery Lane vor dem Tor. In der Party-Welt steht das Tor offen; ein Zaun läuft um die Zone (`zoneFence`: Kapseln auf dem Rechteck in 1-m-Schritten, ohne die Stücke in einer Halle oder Landmarke, deren Wand die Zone dort schließt), die Weltgrenze ist die Zone. `MapData.fences` führt die Zaunstücke mit `party: true` (M4 zeichnet sie nur in der Party), dazu `partyZone` und `arenaBounds`. Die Last-Bots behalten ihre Arena-Ziele: aus den Höfen hat keines einen freien Weg, der freieste Weg führt die Straßen und Gassen entlang (360–600 m in 30 s von jedem Hof-Spawn). Die Validierung prüft Zone (enthält die Arena, in der Kartengrenze), Hof-Spawns (in der Zone, auf einer Straße, trocken) und Hof-Items (in der Zone, auf einer Straße, nicht in der Arena). | Auftrag des Schritts (Coins, Power-ups und Spawns in der Arena und im angrenzenden Hafen); 180 × 140 m allein waren für 16 Spieler eng, und der Hafen liefert Deckung durch die Hallen. Ohne `party` in `pois.json` bleibt das alte Verhalten (Arena hinter geschlossenem Tor). |
| A55 | 4, 13.3 | **Alle sechs Strecken im Rennbetrieb.** `TRACK_IDS` = Downtown Loop, Coast Sprint, Ridge Climb (`hill-sprint`), Harbor Circuit, Dune Rally, Grand Tour; Rotation („nächste Strecke“) in dieser Reihenfolge, die Lobby zeigt sie in drei Spalten (zwei auf schmalen Handys). Die Bonus-Strecken sind dabei, weil Bots sie sauber durchfahren. Steigung entlang der Ideallinie: befestigt ≤ 15 % wie in Phase 2, unbefestigt ≤ 21 % (A14: Erde 18 %, Sand 20 %, plus 1 % Bake-Toleranz). Ghosts: `GhostStore.retain` beim Serverstart; `GHOST_PERSONAL_MAX` 64 → 20, damit sechs Strecken mit 150-s-Läufen in die 5 MB des Stores passen. | Abschnitt 4 plante T5/T6 als „später“; beide bestehen die Datentests und den Bot-Integrationstest. Der Store rechnete mit zwei Strecken. |
| A56 | 13.2 | **Rundkurse beginnen am Start, Gates „voraus“.** `routeToTrack` legt den Anfang der Mittellinie eines Rundkurses auf den letzten Punkt vor dem Start/Ziel-Gate (wie der Downtown Loop in Phase 2). `gateArcLengths` nimmt je Gate den ersten Kandidaten voraus vom vorigen Gate (`projectGlobal(…, ahead)`), nicht den entlang der Linie nächsten. `trackVersion` von Downtown Loop (4) und Harbor Circuit (3) + 1. | Beim ersten Rennen mit fünf Bots gefunden: Die Gates lagen entlang der Linie außer der Reihe (Rundkurse begannen an ihrer ersten Kante), Beinlängen wurden negativ, und jeder Bot setzte sich ewig vor einem „verpassten“ Gate zurück. Auf der Dune Rally lag Gate 3 auf dem Slalom-Ast dahinter (24 m daneben). Der Einzelfahrer-Test (`driveTrack`) sah das nicht, weil er nicht über die Gates des Rennmodus lief. |
| A57 | 10.3 (Phase 2), 14 | **Bots fahren sich nicht fest.** Ein Renn-Bot will immer fahren: Stillstand zählt für die Stuck-Logik auch mit leichtem Gas (hinter einem stehenden Auto verlangte er Tempo 0 und wartete ewig; zwei verkeilte Bots warteten aufeinander). „Losgefahren“ heißt 1 s schneller als 4 m/s (`GOING_TICKS`), sonst setzte ein Abprallen die Back-offs zurück und der Reset kam nie. Traktionskontrolle: kein Gas, solange die Geschwindigkeit mehr als 0,12 rad von der Fahrzeugachse abweicht (ab 5 m/s): 192 statt 92 Resets in 120 Rennen, 1–2 % langsamer. Ein Reset, der mehr als 50 m hinter dem eigenen Platz auf der Linie landet (`RESET_BEHIND_MAX`, ein Nachbarast), setzt das Auto dorthin zurück (Regel des Servers wie 10.3, auch im Ghost-Replay; im Sim-Hash). Vorher: 6 DNF und Stillstände bis 26 s in 60 Rennen, danach keiner und höchstens 9,6 s in 120. | Der Bot-Integrationstest je Strecke verlangt Rennen ohne Festfahren. |
| A58 | 15 | **E2E-Laufzeit.** Die Pfade Touch, Rennen auf dem Handy und fehlende Assets zeichnen 5 Bilder pro Sekunde (`FLOW_DRAW_FPS`, `?drawfps`); Spiel, Netz und HUD laufen weiter jedes Bild. Maß: die CPU-Zeit des ganzen Laufs lokal, die der Laufzeit auf dem 4-Kern-Runner folgt (main: 1089 CPU-s lokal, 4,9 min in CI). Jetzt 829 CPU-s (vorher auf dem Branch 1017), also ca. 3,8 min Tests im Job. Verworfen: Pixelverhältnis 1 im Software-Tier (kein Gewinn) und Software-Compositing (`--disable-gpu-compositing`: GPU-Prozess fast still, aber das Zurücklesen des WebGL-Canvas blockierte die Seite, die zwei Spieler verloren Eingaben). | Der E2E-Job lag auf main bei 5:26 (Ziel < 5 min, CLAUDE.md). |
| A59 | 9, 10, E7, A32 | **Gelände als Clipmap auf der CPU** statt CDLOD mit Morphing im Vertex-Shader. `terrainGrid.ts`: verschachtelte quadratische Ringe um die Kamera (Ebene L mit Raster base · 2^L, 2 · `half` Zellen breit, Loch für die innere Ebene), jede Ebene ein Mesh, dessen Ecken auf der CPU aus `heightAt` gefüllt werden, wenn die Kamera sich 3 Raster von ihrer Mitte entfernt hat (Hysterese). Randecken zwischen zwei groben Ecken nehmen deren Mittel: keine Risse. Jenseits der Kartendaten steigen Hügel an (`horizonHeight`, nur Optik). Stufen: high 0,5 m × 64 × 8 Ebenen, mid 0,5 × 48 × 8, low 0,5 × 32 × 9, software 1 × 24 × 8 (reicht 3–4 km). Gemessen gegen den Boden der Sim: 4,3 cm bei 0,5 m auf dem rauesten befahrbaren Boden, 2,5 cm auf Straßen (Validator), 69 cm bei 2 m. Der Review-Befund (3,3 m zwischen gezeichnetem und simuliertem Boden der alten Stadt) ist damit behoben; `tests/client/terrainGrid.test.ts` misst es. Oberflächen als Splat aus der gebackenen Schicht (`terrainSplat.ts`, zwei RGBA-Texturen, ein Texel je Rasterpunkt). | Jede Ecke liegt exakt auf dem Boden der Sim (kein Morphing zwischen Ebenen, das Autos schweben ließe); kein Vertex-Texture-Fetch von Float-Texturen (iOS); das Nachfüllen einer Ebene kostet im Median 0,17 ms (high, in Node gemessen) und fällt nur alle paar Meter an. |
| A60 | 10 | **Vierte Detailstufe „mid“** und Detailstufen getrennt vom Render-Tier (`worldQuality.ts`): high (Desktop), low (Handy), software (CPU-Rasterizer), mid für einen Desktop, dessen GPU selbst beim kleinsten Pixelverhältnis zu langsam bleibt (`AdaptiveRenderQuality.struggling`, zwei langsame Messfenster) oder mit `?detail=mid`. Die Stufe regelt Gelände-Ringe, Kit-Zellen, Sichtweiten der Instanzen und die Dichte der Streuung; Licht und Materialien bleiben die des Tiers. | Schwache Desktop-GPUs (integrierte Grafik) bekamen sonst die volle Welt oder den Handy-Look. |
| A61 | 9 | **Meshes je Chunk anders geschnitten.** Gebäude, Landmarken, Container und Pier sind ein Mesh je Kit-Zelle (ein Material, der gemeinsame Atlas; Fenster, Türen und Läden im Atlas) statt `facade`/`roof`/`trim` je Chunk (A62). Straßen, Kreuzungen, Areas, Gehwege und Bordsteine sind ein Mesh je Belag (Asphalt, Beton, Erde, Kies, Sand, Gehweg, Pflaster) und **500-m-Block** statt je 250-m-Chunk; auf dem Software-Tier teilen sich alle Beläge ein Shader-Programm (Texturperiode und Markierungen als Uniforms). Leitplanken, Jersey-Barrieren und Felsen sind Instanzen der Kit-Teile, Zäune und Geländer je ein Mesh. | Die Straßen der ganzen Karte sind nur 45 k Dreiecke: 250-m-Chunks hätten an der Plaza 40 Draw Calls mehr gekostet, ohne Blöcke musste ein CPU-Rasterizer jedes Bild alle 45 k transformieren. Ein Programm spart dem CPU-Rasterizer die Kompilierung von fünf weiteren. |
| A62 | 9, 10 | **Kit-Zellen als Quadtree, gebaut bei Bedarf** (`kitCells.ts`): 250-m-Chunks mit LOD2, ihre 125-m-Viertel mit LOD1, deren 62,5-m-Achtel mit LOD0; ein Chunk teilt sich unterhalb von `lod1` (high 180 m, Handy 110 m), ein Viertel unterhalb von `lod0` (high 60 m, Handy 50 m) mit Hysterese; jedes Teil wird genau einmal gezeichnet. Handys nehmen nah LOD1 („ohne Gesimse“; *seit A81 nur in den 62,5-m-Achteln, die 125-m-Viertel LOD2*), der Software-Tier LOD2 in 125-m-Zellen bis 220 m. Beim Laden entstehen alle LOD2-Chunks; nähere Zellen werden bei Bedarf gemergt (höchstens drei je Bild, solange zeichnet der Chunk seine LOD2) und nach 600 Bildern ohne Nutzung weit außer Reichweite wieder verworfen. Nur Zellen im Schattenbereich werfen Schatten. | „Alle Chunks beim Start bauen“ hätte 549 Gebäude in drei LODs gemergt (Speicher, Ladezeit auf dem Handy); so kostet der Start nur die LOD2-Chunks. |
| A63 | 9 | **Instanzen je Chunk** (`chunkedInstances.ts`): ein `InstancedMesh` je Art für die ganze Karte, die Instanzen nach Chunk sortiert; gepackt werden die der sichtbaren Chunks innerhalb der Sichtweite der Art (je Instanz geprüft), neu bei geändertem Sichtbarkeitsmuster oder wenn die Kamera eine andere 8-m-Zelle betritt (statt „höchstens alle 250 ms“). Eine Art kann ein Nah- und ein Fernmodell haben (zwei Instanzbänder mit anschließenden Bereichen). Münzen und Power-ups (51 und 33 in der Party) sind ebenfalls instanziert: vorher kosteten sie 95 von 131 Draw Calls hinter dem Splash. | Gleiche Kosten ohne Timer, und ein Sprung der Kamera zeigt nie eine leere Umgebung. |
| A64 | 10, 15 | **Kosten auf dem CPU-Rasterizer.** Mit der Karte kostete ein SwiftShader-Bild hinter dem Splash 1 690 statt 1 180 CPU-ms (CPU-Zeit des GPU-Prozesses je Bild an festem Punkt), die E2E-Suite 1 038 statt 835 CPU-s. Dagegen: ein halbes Pixel je CSS-Pixel und Achse auf einem CPU-Rasterizer (Pixelverhältnis fest 0,5 statt adaptiv 0,75–2: ein Drittel weniger Zeit je Bild an der Plaza, ein Fünftel am Splash), kein Multisampling dort (ein Wegwerf-Kontext erkennt ihn, bevor der Renderer entsteht; ein Drittel jedes Bildes), nichts unter dem deckenden Ladebildschirm zeichnen, die Instanzierung aus A63, Zaunpfosten mit vier Seiten, Straßen-Blöcke aus A61, Kit-LOD2 in 125-m-Zellen, keine Normal- und ARM-Atlanten transkodieren, keine Mip-Kette für den Splat. Ergebnis: 1 060–1 080 CPU-ms je Bild, E2E-Suite 847 CPU-s (main zur selben Zeit 1 132); gepaart unter Last der Maschine abwechselnd gemessen, mit dem halben Pixelverhältnis: Branch 935 CPU-s / 137 s, main 1 244 CPU-s / 158 s. | Der E2E-Job muss unter 5 min bleiben (CLAUDE.md); main lag bei 5:26. Die Einsparungen gelten auch für Nutzer ohne GPU. |
| A65 | 11.1, 11.2, 8.2 | **Straßenmöbel mit Collidern** (`src/shared/map/furniture.ts`): in Downtown und im Park Laternen alle 36 m auf beiden Gehwegen (mitten zwischen den Palmen der Main Street), an jeder Kreuzung mit Ampel ein Auslegermast rechts jeder Zufahrt 1 m vor der Kreuzungsfläche, je Block ein Hydrant am Ende rechts, auf Gehwegen ab 3 m Bänke mit Mülleimer, dazu acht Bänke im Ring um den Plaza-Brunnen. Collider wie in der alten Stadt: Masten rund (top 5,5 bzw. 7,5 m), Hydrant und Mülleimer niedrig, die Bank eine gedrehte Box; zusammen 220 Teile, 3 596 statt 3 374 Collider. Blöcke halten Trimm-Radius + 4 m frei, hinter einer weiter getrimmten (spitzen) Kreuzung den Überschuss noch einmal. Der Client zeichnet die G1-Modelle bis 70 m (Handy 35 m, weniger Segmente), dahinter wenige Boxen. | Tabelle 11.1 verlangt die Möbel, die alte Stadt hatte sie. Hydranten direkt hinter einer Kreuzung und Laternen am Ausgang der spitzen Diagonal-Kreuzungen lagen im Auslauf abbiegender Renn-Bots (ein Bot stand 12 s fest); mit den Regeln oben: 120 Rennen über alle Strecken ohne DNF, längster Stillstand 573 Ticks (vorher 576). |
| A66 | 3.3, 3.5 | **Aussichtspunkt.** Der Parkplatz (y = 130) liegt in einem Einschnitt unter dem Gipfel: aus Augenhöhe gibt es in keiner Richtung Sicht auf die Bucht, nur entlang der Zufahrt. Die Münzfernrohre stehen deshalb auf der Kuppe (143 m) zwischen Parkplatz und Bucht, wo der Blick frei ist (`viewpoint.ts`: höchster Punkt auf dem Weg zum Pier, 20–90 m vom Parkplatz), als kleine Requisiten ohne Collider wie die Sonnenschirme. Die Screenshot-Ansicht `lookout` steht dort. *Erledigt mit A76:* Die Kuppe ist im Bake zur Bucht hin abgetragen, die Fernrohre stehen am Geländer des Parkplatzes. | Das Gelände zu ändern hätte Sim-Boden, Strecke und Bot-Kalibrierung dieses Schritts mitverändert. |
| A67 | 11.1 | **Requisiten der Zonen ohne Collider** (`scatter.ts`, nur Optik wie in 11.2): ein Briefkasten vor jedem Haus in Seaview Heights (203), Holzzäune entlang der Wege durch die Ranch (1 255 Pfosten, offen an Kreuzungen, unterbrochen an Straßen, Losen und Gebäuden), Palettenstapel in den Hafenhöfen (71), Volleyballnetze am Strand (2) und dichtere Sonnenschirme (19). Nicht umgesetzt: Poller und Kaikante (*Stand: der Kai mit Pollerkette, A77*), Zaun-Reste in den Dünen, Weidezäune in den Hügeln (die Ridge Road hat talseitig Leitplanken). | Die Tabelle nennt diese Requisiten; der trockene Sand des Strandes ist ein schmaler Streifen (11 000 m²), dort passen nur wenige. |
| A68 | 3.5, 11.1, A47 | **Hafenkräne:** ein Drehkran auf einem 8 × 8-m-Betonsenkkasten als Landmarke im Kit (`landmark_quay_crane`, 894/510/268 Dreiecke), der Ausleger 20 m über das Wasser; der Senkkasten ist der Collider (`LANDMARK_PIECES.crane`). Die Standorte aus `pois.json` liegen an der Wasserlinie auf nassem Sand, 24–30 m neben der PCH (*Stand: auf dem Kai, A77*). | Ein Portalkran bräuchte einen Kai, den das offene Hafenbecken nicht hat (3.6). |
| A69 | 14.2 | **M4 geht mit dem Client-Teil von M5 live**, ohne `?map=bulli-bay` und Dev-Flag: Der Client zeichnet nur noch Bulli Bay, die alte Stadt ist gelöscht (`cityGen`, `worldGen`, `colliderGen`, `props`, `city.ts`, `environment.ts`, `streetLayout.ts`, der `RoadGrid`-Reset, die Fassaden- und Ladenfront-Texturen); Radar und Streckenkarte sind genordet (E2). | Nutzerentscheidung: neue Features gehen direkt live, die neue Karte ersetzt die alte Stadt. |
| A70 | 7, 10 | **Horizont und Wasserlinie.** Das offene Meer zeigt den Horizont, den die alte Stadt hinter Hügeln versteckte. Die Wolkenebene beginnt erst oberhalb der Klemmung ihrer Projektion (d.y 0,035–0,08): darunter hingen senkrechte Streifen. Der nasse Sand dunkelt über ±15 cm um den Wasserspiegel ab statt in einer Stufe, und das Meer blendet über seine ersten 12 cm Tiefe ein: Das Watt liegt Zentimeter um y = 0, eine harte Kante folgte dort den Dreiecken in Blöcken. | Beim Durchsehen der Screenshots (Sonnenuntergang, Aussichtspunkt, Überblick) gefunden. |
| A71 | 10, A64 | **Software-Tier ohne Schattenkarte** (`LIGHTING.shadow.enabled`): Auf einem CPU-Rasterizer kosteten Schatten-Pass und die 3 × 3-Abfragen in jedem beleuchteten Pixel rund ein Siebtel jedes SwiftShader-Bildes (Main Street 481 → 411 CPU-ms je Bild) und die Tiefen-Programme eine halbe Sekunde Kompilieren je Seite. Die Kontaktschatten unter den Autos bleiben; high und low behalten ihre Schatten (der Render-Job prüft `shadowCalls > 0` auf low). | Der Software-Tier dient CPU-Rasterizern, also den E2E-Läufen und Rechnern ohne GPU; beiden hilft die Bildrate mehr als der Schatten. |
| A72 | 10, 15 | **Budget je Tier an drei Stellen:** software (110 Calls, 300 k Dreiecke) im E2E-Desktop-Pfad an fester Ansicht, low (150 / 500 k) im Render-Job, high (300 / 1,2 Mio.) nur als Messung: `npm run screenshots` schreibt Tier, Calls und Dreiecke je Ansicht in `stats.json` und warnt über jeder Grenze (auf dem M5-GPU alle 45 Ansichten im Budget: Desktop bis 184 Calls und 1,12 Mio. Dreiecke, Handy bis 126 Calls und 483 k Dreiecke). Die Minimap-Achsen prüft ein Unit-Test (`radar.test.ts`) statt E2E. | SwiftShader bräuchte für den Desktop-Tier (Umgebungskarte, LOD0, 2048er-Schatten) mehrere Sekunden je Bild; das sprengt die 5 min des Render-Jobs. Die Achsen sind reine Logik und gehören auf die niedrigste Ebene. |
| A73 | 15 | **E2E-Job wieder unter 5 min.** Gemessen in einem Linux-Container mit 4 CPUs (wie der CI-Runner, Playwright-Image 1.63, Zeiten der Playwright-Läufe, je zwei bis drei Läufe): main 2,6–3,1 min (in CI 4,9 min Tests, Job 5:26), der Branch vorher 2,4 min, jetzt 1,7–1,9 min (fünf Läufe). Hochgerechnet mit dem Verhältnis zu main (0,62–0,73) etwa 3,0–3,6 min Tests und 3,6–4,3 min Job in CI; der Render-Job 1,6–1,7 statt 2,1 min im Container (main in CI 4,3 min Tests, Job 5:01) → etwa 4,2 min Job. Maßnahmen: (1) Der Zwei-Spieler-Test wartete mit einem kurzschließenden `every()` eine ganze Lag-Fenster-Länge (5 s) zu spät auf die zweite Seite. (2) Beide Seiten laden bis zum Splash, bevor ein Auto fährt: Das Laden einer Seite blockiert die andere im gemeinsamen GPU-Prozess, und ein fahrendes Auto ohne Inputs wurde zum Lag-Ghost, der bis zu 10 s zum Erholen braucht. (3) Die mittlere Bildfarbe liest der Test-Hook aus dem Zeichenpuffer direkt nach dem nächsten Bild statt per Screenshot (auf SwiftShader bis 2,7 s Warten auf den Compositor; gleiche Werte, schwarze Platzhalter geben 63,8 wie vorher 64). (4) A71. (5) Der Neustart-Pfad zeichnet mit 5 Bildern je Sekunde. Verworfen: die Grace-Zeit des E2E-Servers von 3 auf 2 s (spart 2 s, aber die Wiederaufnahme nach einem kurzen Abbruch braucht bis 0,6 s Backoff plus Handshake auf einer ausgelasteten Seite; der Spielraum wäre unter 1,5 s gefallen). | Die lokale Uhr des M-Mac (18 Kerne, fremde Last) sagt wenig über den 4-Kern-Runner; der Container wurde mit main gegen die CI-Zeiten von main kalibriert. |
| A74 | 7, 10 | **Beton, Gehweg und trockener Sand lesen sich im Abendlicht vom Boden ab.** Unter der Sonnenfarbe 0xffbd88 kamen Beton (Lots, Hafenstraßen, Arena) und Gehwege im selben Terrakotta an wie die Erde (ΔE 13 zwischen Tankstellen-Lot und Gras, 5 zwischen Strand und Gras). Jetzt: Beton und Gehweg mit der Tönung (1,1, 1,33, 1,42) auf der Gehweg-Textur, die Flecken nur noch schwach warm (0,86, 0,85, 0,83), auf dem Software-Tier die flachen Farben 0xc8e2e6/0xcee6ea; der trockene Sand im Gelände-Shader × (1,3, 1,45, 1,55), die Erde dunkler und röter (0,62, 0,47, 0,35); Fels grau statt im Farbton des Grases (80 % entsättigt, leicht kühl), zwei Texturskalen (23 und 7 m), Schichtbänder von 2 bis 6 m und Hell-Dunkel über 30 bis 60 m. Gemessen (senkrecht von oben, 12 m, M5-GPU): Lot Lab (51, 5, 16) gegen Gras (27, 12, 21), ΔE 25; Sand (46, 12, 20), ΔE 18. Der Render-Job prüft auf dem Handy-Tier ΔE > 18 (Lot/Gras) und > 15 (Sand/Gras), gemessen 26 und 20, vorher 13 und 12. Das Plaza-Pflaster bleibt Saltillo-Terrakotta. | Befund „ein einziger Braunton“ des Art-Reviews; im goldenen Licht muss ein heller Stoff ins Blaugraue getönt sein, um neutral anzukommen. |
| A75 | 6.4, 7 | **Klippen als Fels statt Böschung.** Die glatte 40-m-Wand war zum größten Teil die 1 : 1,5-Böschung der Küstenstraße und des Aussichtspunkts, die das Bake die Klippe hinunter schüttete (im Schnitt 30 m von 5 auf 37 m). Jetzt: (1) Auf der Klippenfläche und im Meer davor füllt das Bake keine Böschung mehr (`BaseSample.cliff`, `TerrainShaper.finish(noFill)`), außer bis 8 m neben der Fläche einer Straße; (2) `vary` in `base.json`: die Kante kommt bis 45 % der Flächenbreite heraus, der Fuß geht in Rinnen bis 3–4 m ins Land und auf den Landzungen 12–14 m ins Meer; die Höhe schwankt um bis zu 50 % (zwei Oktaven Rauschen über 65–75 m), voll auf der Fläche, über 30–35 m landeinwärts zurück zur Klippenhöhe; (3) die Variation bleibt 3 m von jeder Mittellinie weg (`terrainKeep`: das Profil einer Straße liest den Boden nur dort) und aus Areas ohne feste Höhe; (4) Felsbrocken am Fuß jeder Felswand (`talusRocks`: 6-m-Raster, wo Fels 4 m höher binnen 8 m steht, bis in 0,8 m tiefes Wasser; 69 Stück mit Collider). Nordklippe gemessen: im Schnitt 12 m von 5 auf 37 m (vorher 30), die Kante 11–31 m vom Profilanfang (vorher 14,5–17,5), der Kamm 40,5–52 m (vorher überall 42). Dafür Leitplanken an `pch-cliff-3` (0–40 m) und am Ende der Ridge Road, wo der Boden jetzt neben der Straße abfällt. | Befund „glatte, gleichförmige Wand mit Strohtextur“; die Straßen auf dem Plateau behalten ihr Profil (Test `bakeTerrain at a cliff`). |
| A76 | 3.3, A66 | **Aussichtspunkt mit Blick.** `views` in `base.json`: ein Keil vom Ostrand des Parkplatzes zum Pier (halbe Breite 0,5 · Abstand, 380 m lang), in dem der Boden unter 133 m − 0,1 · Abstand bleibt, sanft auslaufend, außer auf den Mittellinien der Straßen. Die Kuppe zwischen Parkplatz und Bucht ist damit um bis zu 12 m flacher; von der Südwestecke des Parkplatzes (Augenhöhe 1,5 m) und von allen vier Spawns (Kamerahöhe) liegt das Meer frei. Die Münzfernrohre stehen jetzt 1 m innerhalb des Geländers in der Ecke zur Bucht (`railSpots`), die Spawns im Westteil des Parkplatzes mit Blick zum Pier (yaw −1,05). Das Geländer reicht auf der Westseite bis 12 m an die Einfahrt (zusätzliche Ecke im Polygon), weil der Boden dort abfällt. Der Parkplatz bleibt auf 130 m, die Ridge Road und das Ziel des Ridge Climb unverändert. | Befund „Aussichtspunkt ohne Aussicht“; ein Abtrag mit festem Boden (statt Keil) hinterließ neben Straße und Parkplatz steile Wände. |
| A77 | 3.5, 11.1, A20, A67, A68 | **Hafen mit Kai und Mole.** Ein Kai als Area (`quay`, Beton, y = 1,42 auf Straßenhöhe, `walls`) zwischen PCH und Wasser (z 445–565, 22–34 m breit), die beiden Kräne darauf (Kran 2 von z 575 nach 545), an der Wasserseite eine Pollerkette als neue Geländerart `bollard` (Guss-Poller alle 3 m mit Kette, Collider 0,7 m hoch), Laternen wie auf den anderen großen Lots. Eine Area mit fester Höhe wird jetzt eben gezeichnet (an ihren Wänden neigte sich der Rand zum Gelände darunter) und zeigt ihre Wände 4 m tief. Der Leuchtturm steht am Ende einer 45 m langen Mole (`base.json` `moles`: ein Damm bis 1,6 m über dem Meeresboden; `pois.json` `moles`: Felsbrocken auf der Krone und Steine an den Flanken, 51 Stück mit Collider). Keine Boote: Das Kit hat keine. | Befund „Hafen ohne Kai“; der Kai folgt dem Vorschlag, die Mole der Alternative für den Leuchtturm. |
| A78 | 5.2, 15 | **Zufahrt der Arena.** Der Knoten `arena-gate` liegt auf dem Rand des Platzes (z = 520 statt 514); der Validator verlangt, dass jeder Knoten einer Area in ihr oder höchstens 1 m vor ihrem Rand liegt. Die Stellplatzlinien beginnen 1,4 m innerhalb des Rands (der Zaun steht 0,5 m innen). | Befund „Zufahrt endet 6 m vor dem Platz“. |
| A79 | 14 (Phase 2), 15 | **Bots auf unbefestigtem Belag, `driveTrack` wie der Raum.** Die leichten Bots planen Kurven auf Erde, Kies und Sand mit 70 % des Grips (`BotSkill.unpavedGrip`; mittel und schwer 100 %): Sie schlingerten mit ihrer Reaktionszeit von 12 Ticks bei 39 m/s aus einem 180-m-Bogen der Dune Rally und verpassten Tor 7. Über 900 Rennen (6 Strecken, 3 Stufen, 5 Klassen, 10 Seeds) verpasste Tore 14 → 5 (die der leichten Bots auf unbefestigtem Belag 10 → 0), Resets 20 → 10. Ein geringerer Anteil auch für die mittleren Bots staute fünf von ihnen im Strand-Slalom der Dune Rally (`trackRaces`, bis 699 Ticks ohne Fortschritt). `driveTrack` hält bei einem verpassten Tor Reset und setzt das Auto vor das Tor zurück wie `RaceRoom` (vorher meldete es ein DNF mit Dutzenden Resets), und zählt verpasste Tore. | Befund des Gameplay-Reviews. |
| A80 | 6.2, 10 | **Modelle komprimiert ausgeliefert.** `/models` (Autos und Kit, meshopt-GLBs) geht wie die HDRIs und das Terrain über `compressedAssets`: Brotli bringt die 2,09 MB des Kits auf 0,84 MB. | Befund des Performance-Reviews; weder Express noch Cloudflare komprimieren GLBs. |
| A81 | 11.2, 10, A62 | **Eckhäuser an den Zeilenenden, Handy-LOD.** Downtown-Zeilen beginnen und enden an Kreuzungen und Straßenenden mit einem Eckhaus (drei neue Kit-Teile, `downtown_corners`), dessen Seiten auf LOD0 Schaufenster und Fenster zeigen; die erste Zeile rückt bis 0,25 m an die Ecke. Damit stehen 562 statt 549 Gebäude, die Main-Street-Ansicht des Handy-Tiers kam auf 544 k Dreiecke. Auf LOD1 und LOD2 bleiben die Seiten der Eckhäuser deshalb geschlossen, und Handys zeichnen die 125-m-Viertel mit LOD2 (`midLod`), LOD1 nur in den 62,5-m-Achteln um die Kamera: 469 k (vorher 477 k). | Die Zeilen endeten mit fensterlosen Brandmauern an den Kreuzungen; das Budget von 500 k (10) gilt weiter. |
| A82 | 3, 5, 8, 11 | **Weitere Korrekturen aus dem Review.** Die Renn-Kamera im Hochformat bleibt unter 5 m (die Signalarme hängen ab 5,06 m); eine Seite, deren Karte nicht zur ausgelieferten `mapVersion` passt, lädt einmal neu statt endlos zu versuchen; das Geländenetz unter dem Pier liegt auf dem Strand statt zum Deck zu steigen (nur Optik); die Geländer des Piers laufen an seinem Fuß 5 m weiter und 1,8 m nach außen, der Validator hält offene Geländerenden aus der Fahrbahn; hinter jeder Sprungschanze bleiben 30 m Anlauf frei von Gebäuden und Pflanzen, der Validator prüft ihn (Karte, Wasser, Leitplanken); große Lots (ab 3 000 m², außer der Arena) haben Laternen alle 30 m am Rand; Barrierenreihen an spitzen Abzweigen rücken in den Abzweig, bis sie die Fahrbahn der Strecke nicht berühren; der Grenzzaun bleibt stehen, wo der Boden unter einem Stück irgendwo über der Wattiefe liegt; Bäume nicht auf Boden steiler als 35°, Büsche nicht über 45°; die Konservenfabrik trägt ihr eigenes Schild, der Surfladen und die Eisenwarenhalle feste Schilder. | Befunde aus demselben Review (Kamera, Deploy, Pier, Sprünge, Licht, Strecken, Grenze, Vegetation, Schilder). |
| A83 | 15, A73 | **E2E-Zeit nach den Review-Korrekturen.** Im Container (Playwright-Image 1.63, 4 CPUs, colima) je zwei Läufe: main 4,1 min (ein Lauf mit dem bekannten Zwei-Spieler-Ausfall von main), der Branch 2,7–2,9 min; Verhältnis 0,66–0,70. Hochgerechnet auf CI (main 4,9 min Tests, Job 5:26): etwa 3,2–3,4 min Tests und 3,8–4,0 min Job, unter den 5 min von CLAUDE.md. Der Render-Job hat mit der Farbprüfung (A74) auf derselben Seite einige Sekunden mehr, lokal 1,2 min. | Die Messung von A73 lief in einer anderen Container-Konfiguration; maßgeblich ist das Verhältnis zu main in derselben. |
| A84 | 10, A64 | **Lite-Grafik (#17) mit Bulli Bay.** Nach verlorenem oder verweigertem WebGL-Kontext nutzt eine Seite sieben Tage den Software-Tier (`render/safeMode.ts`); mit Bulli Bay heißt das auch die Software-Weltdetailstufe (Kit LOD2 bis 220 m, Terrain-Splat ohne Mip-Kette, keine Normal-/ARM-Atlanten). Die Pixeldichte bleibt dabei 1 (Untergrenze 0,75) statt der 0,5 der CPU-Rasterizer, und die Wegwerf-Kontext-Prüfung für MSAA (`wantsAntialias`) entfällt, weil Lite-Grafik ohnehin ohne MSAA läuft. | Rebase auf main nach #17: Lite-Grafik soll auf einer echten GPU Speicher sparen, nicht unscharf sein; ein zusätzlicher Kontext auf einem Gerät, das gerade Kontexte verliert, wäre das falsche Signal. |
| A85 | 15, A73 | **CI-Zeiten nach dem ersten PR-Lauf.** E2E-Job 3:50 (main 5:40). Der Render-Job lief in das 120-s-Limit des Handy-Tier-Tests: SwiftShader braucht auf dem CI-Runner je Bild des Handy-Tiers Sekunden (lokal 0,1 s), die Bodenfarben-Prüfung kostete dort 45 s. Die Bilder sind pixelgebunden; der Test läuft jetzt in 320 × 200 statt 640 × 400 (gleicher Bildausschnitt, also gleiche Draw Calls und Dreiecke; LODs und Impostoren gehen nach Entfernung, Handy-Kamera und HUD nach der Breite unter 768 px), im 4-CPU-Container 30 statt 42–46 s. Der Unit-Job stieg mit den Karten-Tests von 53 auf 113 s (`npm test` 24 → 69 s): Die Logik-Tests (`tests/shared`, `tests/server`, `tests/tools`) teilen sich jetzt die Module eines Workers (`isolate: false`, Projekt `logic` in `vitest.config.ts`), so entsteht die Karte einmal je Worker statt je Datei; die Client-Tests bleiben isoliert (ohne Isolation schlugen `worldRestore` und `mapScene` in einer von acht gemischten Reihenfolgen fehl, die Logik-Tests in keiner von 16). Die Tests, die die ganze Karte neu bauen, stehen in `mapVariants.test.ts` neben `mapData.test.ts`. Im Container 19 → 13–16 s. Stryker behält eine eigene, isolierte Konfiguration. Im Integrationstest des Rennens schickt der manipulierende Bot seine Pakete jetzt alle 5 Ticks ab dem bekannten Start-Tick (vorher nur in den letzten 200 Ticks alle 10; neben dem Zeitfahren sah er auf dem Runner einmal nur zwei Gelegenheiten). CI danach: Unit-Job 56 s, E2E 3:52, Render 3:21, Bots 3:05. | Die Budgets aus CLAUDE.md (E2E und Render je < 5 min, Unit < 1 min, Integration < 5 min). |
