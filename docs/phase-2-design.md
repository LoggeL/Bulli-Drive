...
### 25.4 Nachbesserungen aus dem Review (Fairness, Last, Testlücken)

| Stelle | Vorher | Jetzt | Grund |
|---|---|---|---|
| Verkehr der Server-Bots (14, 11) | alle lebenden Renner | **ohne Race-Ghosts** (Falschfahrer, im Ziel, DNF); im Start-Ghost sehen die Bots weiter alle | Die Bots wichen Ghosts aus, die sie gar nicht treffen können: Ein entgegenkommender Falschfahrer zog sie 120 m vorher zur Seite, ein im Ziel außen in der Kurve geparktes Auto bremste jeden Bot fast bis zum Stillstand (Griefing nur gegen Bots). Getestet über `RaceRoom.trafficStates` (was die Bots sehen); das Ausweichen selbst testet `lineDriver.test.ts`. |
| Lag-Ghost im Rennen (11) | 1b-Regel unverändert (RTT > 300 ms oder > 20 % fehlende Inputs) | Für Renner vom Countdown bis zu den Ergebnissen **nur fehlende Inputs**; solange ein Renner Lag-Ghost ist, bekommt jeder fehlende Tick den **Stopp-Input** statt der Wiederholung des letzten (Room-Hooks `repeatsMissingInput`, `rttMakesLaggy`) | Ein eigener Client ließ jeden 4. Input weg: Die Wiederholung machte die Lücken kostenlos, das Auto fuhr gleich schnell und war trotzdem dauerhaft unrammbar. Jetzt kostet jede Lücke Tempo (nach 200 Ticks 8 statt 25 m/s). Ein zurückgehaltener Pong kostet nichts und macht deshalb im Rennen keinen Ghost mehr; kommen die Inputs rechtzeitig, rechnet der Server das Auto bei jeder RTT richtig. Party und Free Roam behalten die 1b-Regel. |
| RETRY im Zeitfahren (6.2) | in jeder Phase sofort ein neuer Countdown, jedes Mal `ghostData` | RETRY wird **im Countdown ignoriert**; `ghostData` geht nur, wenn der Client diesen Lauf noch nicht hat (verglichen über Key, Spieler, Zeit und Aufnahmezeit; neu nach Streckenwechsel und Resume, weil der Client ihn dort verwirft) | 20 RETRY/s pro Sitzung erzeugten je 31 kB Downlink und bei Cache-Fehlgriff eine Nachsimulation im Tick (nachgestellt: 17 Sitzungen, 1 s Spielzeit in 4,6 s Wanduhr). Jetzt: 3 `ghostData` statt 340, 63 ms. |
| Pose-Spuren (15.3) | Prüf-Replay im Ziel, beim nächsten Countdown ein zweites Replay | Die Pose-Spur des Prüf-Replays geht mit `submit(run, poses)` in den LRU; `cachedPoses` gibt nur Vorhandenes heraus. Fehlt die Spur, wartet der Versand auf das **Replay-Budget des Prozesses** (`ReplayBudget`, 2 Replays pro Sekunde, 2 auf einmal) und kommt in einem späteren Tick | Pose-Spuren an jedem Lauf zu speichern, hätte das 5-MB-Budget gesprengt (6,4 MB im Grenzfall, 25.2). Das Budget begrenzt die Replays im Tick auf etwa 40 ms pro Sekunde, egal wie viele Sitzungen Ghosts außerhalb des Caches anfordern. Der Client baut den Ghost auch, wenn `ghostData` nach dem Countdown-Beginn kommt. |
| Leere Race- und Zeitfahr-Rooms (6.5) | schließen 60 s nach dem letzten Spieler (`sweep`) | schließen **sofort**, wenn der letzte Spieler geht (Wechsel, Reload, Verbindungsende) | Mit `fresh` bzw. jedem Zeitfahren konnte jede Sitzung alle 2 s einen Room öffnen und stehen lassen (nachgestellt: 4800 Rooms). In einen leeren Race-Room kommt niemand zurück, in einen Zeitfahr-Room nie. Jetzt höchstens ein Room pro Sitzung; Party und Free Roam behalten die TTL. |
| E2E-Renn-Test (20.3) | … REMATCH, dann über den Room-Chip zurück in die Party | endet nach REMATCH mit dem nächsten Countdown | Der Rückweg über den Room-Chip steht schon in `mobile.spec.ts`, das Ausblenden des Race-HUD in `raceUi.test.ts`. Das spart im E2E-Job einige Sekunden. |

**Gemessen (lokal, macOS arm64):** `npm run test:e2e` 2:33–2:34 min (8 Tests inkl. Build), davon der Renn-Test 24,0–25,4 s; `npm run test:bots` 2:31–2:36 min; Unit-Lauf 1375 Tests in 5,1 s. **CI:** Für diesen Branch gibt es noch keinen Lauf. Die Zeit des Jobs „End-to-end (Playwright)“ gehört in den PR. Auf `main` schwankt der Job schon ohne den Renn-Test stark (Lauf 35993990953: 5:05 min, davon `npm run test:e2e` 4:17 min; andere Läufe 3:33 bis 8:04 min). Liegt er im PR über 5 min, ist die Ursache zu suchen, nicht das Ziel anzuheben. Die teuersten Tests sind `two-players` (lokal 36,5 s), `mobile` (28,6 s) und `desktop` (22,0 s); das gehört in einen eigenen Schritt.

**Tests und Mutationsproben:** `raceRoom.test.ts`: Kein Renner trägt nach dem Start-Ghost `CAR_RACE_GHOST` (Probe `ghost = !!racer` wird rot). Race-Ghosts fehlen in `trafficStates`, im Start-Ghost sind alle drin (Proben: Filter weg; `forceGhost` statt Race-Ghost). Split mit drei Rennern gegen den Vordermann (Probe `order[0]`). Genau ein `launch` je Renner auch nach Grün (Probe `T >= S`). 25 % Input-Lücken kosten Tempo (Probe: Wiederholung immer). Die RTT macht im Rennen keinen Ghost, in der Lobby schon (Probe: RTT immer). Ein Bot, der ein Gate umfährt, wird davor zurückgesetzt und fährt durch (Probe: Zeile `missedGate → requestReset` weg). `lineDriver.test.ts`: Ein festsitzender Bot setzt nach zwei Back-offs zurück (Probe: `stuck.watch` weg). `timeTrial.test.ts`: Ein RETRY-Schwall ergibt einen Countdown, derselbe Ghost geht nicht zweimal, nach Streckenwechsel und Resume wieder (Proben: Countdown-Sperre, Dedupe, Reset bei Streckenwechsel bzw. Resume, Pose-Spur beim `submit`). Ein neuer Rekord geht beim nächsten RETRY wieder hinaus (Probe: Lauf-Kennung konstant). Ohne Budget wartet der Ghost (Proben: Budget umgangen, Nachversand im Tick weg). Dazu der Vertrag `submit(run, poses)`/`cachedPoses` im Ghost-Store und ein Test für `ReplayBudget`. `lobby.test.ts`: 20 Wechsel mit `fresh` halten genau einen Room der Sitzung offen (Probe: Schließen weg). Jede Probe wurde rot und danach zurückgesetzt. Stryker über die geänderten Dateien: `ghostStore.ts` 88 % (vorher 85 %), `RaceRoom.ts` 82 % (82 %), `TimeTrialRoom.ts` 79 % (75 %), `lobby.ts` 88 %, `Room.ts` 76 %; auf den geänderten Zeilen überleben nur äquivalente Mutanten (die Schutzabfragen in `sendGhost`, die nur mit gesetztem `ghostDue` und einem Renner läuft).
