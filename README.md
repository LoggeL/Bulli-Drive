...
| `npm run test:e2e:render` | Builds, then runs the render checks of the phone tier in the browser: the draw call and triangle budget, and a world without its textures that is shaded, not black (`tests/e2e-render`, Playwright project `render`) |
