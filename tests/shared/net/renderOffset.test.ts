...
import {
    CONTACT_SMOOTH_MAX_MS, SMOOTH_TAU_MAX_MS, SNAP_DISTANCE
...
describe('RenderOffset', () => {
    ...
    it('fades with tau 100 ms for small offsets and 200 ms from 2 m on', () => {
        // τ = 100 ms + 100 ms · clamp(|offset| / 2 m, 0, 1) (8.4)
        const small = new RenderOffset();
        small.correct(pose(0.1, 0), pose(0, 0), false, 0);
        small.decay(100, 0);
        // 100 ms + 100 ms · 0.1 m / 2 m = 105 ms
        expect(small.tauMs).toBeCloseTo(105, 12);
        expect(small.x).toBeCloseTo(0.1 * Math.exp(-100 / 105), 12);
        const large = new RenderOffset();
        large.correct(pose(3, 0), pose(0, 0), false, 0);
        large.decay(10, 0);
        expect(large.tauMs).toBe(200);
        expect(large.x).toBeCloseTo(3 * Math.exp(-10 / 200), 12);
    });

    it('holds a contact offset under a straight ramp down to 0 at 300 ms, in every axis and in yaw', () => {
        const offset = new RenderOffset();
        // 1.2, 0.8, 1.2 m and -0.4 rad at t = 1000 ms: the ramp starts at
        // its size (1.876 m) and 0.4 rad
        offset.correct(pose(1.2, 1.2, -0.4, 0.8), pose(0, 0), true, 1000);
        const start = offset.size;
        // A 1 ms frame at 1200 ms: the fade alone keeps 99.5 %, the ramp
        // allows a third (100 ms of 300 left)
        offset.decay(1, 1200);
        expect(offset.size).toBeCloseTo(start / 3, 9);
        expect(offset.x).toBeCloseTo(1.2 / 3, 9);
        expect(offset.y).toBeCloseTo(0.8 / 3, 9);
        expect(offset.z).toBeCloseTo(1.2 / 3, 9);
        expect(offset.yaw).toBeCloseTo(-0.4 / 3, 9);
    });

    it('drops the contact ramp when a correction without contact follows', () => {
        const offset = new RenderOffset();
        offset.correct(pose(2, 0), pose(0, 0), true, 0);
        offset.correct(pose(2, 0), pose(0, 0), false, 10);
        // 10 ms before the old contact deadline: only the fade (τ 200 ms)
        offset.decay(1, 290);
        expect(offset.x).toBeCloseTo(2 * Math.exp(-1 / 200), 12);
    });
    ...
    it('is gone below 1 mm, within 1.5 s of 60 fps frames even from 3.9 m', () => {
        ...
        while (offset.active && t < 2000) {
            ...
            expect(last - offset.size).toBeLessThanOrEqual(last * (1 - Math.exp(-(1000 / 60) / 100)) + 1e-3);
            last = offset.size;
        }
...
describe('interpolatePose', () => {
    it('interpolates position linearly and yaw the short way', () => {
        ...
        a.y = 1; b.y = 3; a.z = 4; b.z = 2;
        const out = interpolatePose(a, b, 0.5, createPose());
        expect(out.x).toBe(0.5);
        expect(out.y).toBe(2);
        expect(out.z).toBe(3);
        expect(interpolatePose(a, b, 0.25, createPose()).y).toBe(1.5);
        expect(createPose()).toEqual({ x: 0, y: 0, z: 0, yaw: 0 });
